...
// Fix Winsock header conflict on Windows
// Must include winsock2.h BEFORE mysql.h to avoid conflicts
#if defined(_WIN32)
...
private:
    ...
    std::string database;
    int port;
    std::unordered_map<std::string, std::vector<std::string>> columnCache;
    std::vector<std::string> tablesCache;
    bool tablesCached = false;
...
public:
    ...
    bool columnExists(const std::string& table, const std::string& column) {
        ...
        std::string target = normalizeIdentifier(column);
        for (const auto& col : columns) {
            if (col == column) {
                return true;
            }
            if (normalizeIdentifier(col) == target) {
                return true;
            }
        }
        return false;
    ...
    bool tableExists(const std::string& table) {
        ...
        std::string target = normalizeIdentifier(table);
        for (const auto& tbl : tables) {
            if (tbl == table) {
                return true;
            }
            if (normalizeIdentifier(tbl) == target) {
                return true;
            }
        }
        return false;
    ...
    std::vector<std::string> getTableColumns(const std::string& table) {
        auto cached = columnCache.find(table);
        ...
        try {
            ...
            while (results && results->next()) {
                columns.push_back(results->getString("column_name"));
            }
        } catch (...) {
            columns.clear();
        }
    ...
    std::vector<std::string> getTables() {
        ...
        tablesCache.clear();
        try {
            if (!connect()) return tablesCache;
            std::string query = "SELECT table_name FROM information_schema.tables "
                                "WHERE table_schema = DATABASE()";
            auto results = executeQuery(query);
            while (results && results->next()) {
                tablesCache.push_back(results->getString("table_name"));
            }
            tablesCached = true;
        } catch (...) {
            tablesCache.clear();
    ...
    std::string resolveColumn(const std::string& table, const std::vector<std::string>& candidates) {
        if (candidates.empty()) {
            return std::string();
        }
        ...
        if (!columns.empty()) {
            ...
            for (const auto& candidate : candidates) {
                std::string target = normalizeIdentifier(candidate);
                for (const auto& col : columns) {
                    if (normalizeIdentifier(col) == target) {
                        return col;
                    }
    ...
    std::string findColumn(const std::string& table, const std::vector<std::string>& candidates) {
        if (candidates.empty()) {
            return std::string();
        }
    ...
    std::string resolveTable(const std::vector<std::string>& candidates) {
        if (candidates.empty()) {
            return std::string();
        }
        ...
        if (!tables.empty()) {
            ...
            for (const auto& candidate : candidates) {
                std::string target = normalizeIdentifier(candidate);
                for (const auto& table : tables) {
                    if (normalizeIdentifier(table) == target) {
                        return table;
                    }
                }
            }
